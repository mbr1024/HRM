...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td width="15" height="32"><img src="${pageContext.request.contextPath}/images/main_locleft.gif" width="15"
                                        height="32"></td>
        <td class="main_locbg font2"><img src="${pageContext.request.contextPath}/images/pointer.gif">&nbsp;&nbsp;&nbsp;当前位置：员工管理
            &gt; 员工查询
        </td>
                ...
                <tr>
                    <td class="fftd">
                        <form name="empform" method="post" id="empform"
                              action="${pageContext.request.contextPath}/employee/findEmployee.do">
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td class="font3">
                                        职位：
                                        <select name="job_id" style="width:143px;">
                                            ...
                                            <c:forEach items="${requestScope.jobs }" var="job">
                                                <option <c:if test="${employee.job.id==job.id}">selected</c:if> value="${job.id }">${job.name }</option>
                                            </c:forEach>
                                        </select>
                                        姓名：<input type="text" name="name" value="${employee.name}">
                                        身份证号码：<input type="text" name="card_Id" maxlength="18" value="${employee.card_Id}">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="font3">
                                        ...
                                        <c:forEach items="${requestScope.depts }" var="dept">
                                            <option <c:if test="${employee.dept.id==dept.id}">selected</c:if> value="${dept.id }">${dept.name }</option>
                                        </c:forEach>
                                    </select>&nbsp;
                ...
                <c:forEach items="${requestScope.employees}" var="employee" varStatus="stat">
                    <tr id="data_${stat.index}" class="main_trbg" align="center">
                        ...
                        <td align="center" width="40px;"><a
                                href="${pageContext.request.contextPath}/employee/updateEmployee.do?flag=1&id=${employee.id}">
    ...
    <tr valign="top">
        <td align="center" class="font3">
            <syl:pager
                    ...
                    style="digg"
                    submitUrl="${pageContext.request.contextPath}/employee/findEmployee.do?pageIndex={0}&name=${employee.name}&dept_id=${employee.dept.id}&card_Id=${employee.card_Id }&job_id=${employee.job.id}&phone=${employee.phone}&sex=${employee.sex}"/>
        </td>
    </tr>
</table>
