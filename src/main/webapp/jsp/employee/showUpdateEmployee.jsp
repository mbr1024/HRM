	...
	$(function(){
		...
		$("#employeeForm").submit(function(){
			...
			if ($.trim(name.val()) == ""){
				msg = "姓名不能为空！";
				name.focus();
			}else if ($.trim(dept_id.val()) < 1) {
                msg = "部门不能为空！";
                dept_id.focus();
            } else if ($.trim(sex.val()) < 1) {
                msg = "性别不能为空！";
                sex.focus();
            } else if ($.trim(job_id.val()) < 1) {
                msg = "职位不能为空！";
                job_id.focus();
...
<table width="100%" height="90%" border="0" cellpadding="5" cellspacing="0" class="main_tabbor">
  <tr valign="top">
    <td>
    	 <form action="${pageContext.request.contextPath}/employee/updateEmployee.do" id="employeeForm" method="post">
			<!-- 隐藏表单，flag表示添加标记 -->
		  ...
		  <table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
		    <tr><td class="font3 fftd">
		    	<table>
		    		<tr>
		    			...
		    			<td class="font3 fftd">身份证号码：<input type="text" name="card_Id" id="cardId" size="20" value="${employee.card_Id }"/></td>
		    		</tr>
		    		<tr>
		    			<td class="font3 fftd">性别：
								<select id="sex" id="sex" name="sex" style="width:143px;">
		    			...
		    			<td class="font3 fftd">职&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;位：
		    			 	<select id="job_id" name="job_id" style="width:143px;">
					    			...
					    			<c:forEach items="${requestScope.jobs }" var="job">
					    				<option value="${job.id }" <c:if test="${employee.job.id==job.id}">selected</c:if>>${job.name }</option>
					    			</c:forEach>
					    		</select>
					    </td>
		    		</tr>
			...
			<tr>
				<td class="font3 fftd">
					...
					QQ&nbsp;&nbsp;号码：<input name="QQ_Num" id="qqNum" size="20" value="${employee.QQ_Num }"/>
				</td>
			</tr>
			<tr><td class="main_tdbor"></td></tr>
			...
			<tr>
				<td class="font3 fftd">
					...
					邮政编码：<input name="post_Code" id="postCode" size="20" value="${employee.post_Code }"/>
				</td>
			</tr>
			<tr><td class="main_tdbor"></td></tr>
			...
			<tr>
				<td class="font3 fftd">
					...
					name="birthday" id="birthday" value="<f:formatDate value="${employee.birthday}" type="date"/>" size="40"/>&nbsp;&nbsp;
			...
			<tr>
				<td class="font3 fftd">
					...
					&nbsp;&nbsp;所属部门：
					<select  name="dept_id" id="dept_id" style="width:100px;">
			...
			<tr><td align="left" class="fftd"><input type="submit" value="修改">&nbsp;&nbsp;<input type="reset" value="取消 " onclick="javascript:window.history.back(-1)"></td></tr>
		  </table>
		 </form>
	</td>
  </tr>
